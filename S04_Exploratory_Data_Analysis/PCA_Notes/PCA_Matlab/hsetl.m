function y=hsetl(PandL_data,cl)
% HSETL_SIMPLE ETL for specified confidence level
%
% Function estimates the ETL of a portfolio using the historical simulation approach, 
...
%
% If index value is an integer, VaR follows immediately and then we estimate ETL
%
...
%
   ...
   etl=mean(tail_losses);% ETL
   y=etl;
end
%
% If index not an integer, take VaR as linear interpolation of loss observations just above and below 'true' VaR
%, and take ETL as linear interpolation of corresponding upper and lower ETLs
%
if index-round(index)>0|index-round(index)<0 
        %
        % Deal with loss observation just above VaR to derive upper ETL
        %
       ...
       upper_etl=mean(upper_tail_losses); % Upper ETL
       % 
       % Deal with loss observation just below VaR to derive lower ETL
       %
       ...
       lower_etl=mean(lower_tail_losses); % Lower ETL
        % 
        % If lower and upper indices are the same, ETL is upper ETL
        %
        if upper_index==lower_index
            y=upper_etl;
        end
        %
        % If lower and upper indices different, ETL is weighted average of upper and lower ETLs
        %
       if upper_index~=lower_index
            %
            % Weights attached to upper and lower ETLs
            %
            ...
            % 
            % Finally, the weighted, ETL as a linear interpolation of upper and lower ETLs
            %
            y=lower_weight*lower_etl+upper_weight*upper_etl;   % ETL
        end
    end
